...
df.to_csv(filename+'Meth_WITH_motifs_2000grt1_7.txt', sep='\t')

#Below selects rows from output above, based on counts of modificaitons per bin.  Seclection criteria: <>2 St.devs

df = pd.read_csv(filename+'Meth_WITH_motifs_2000grt1_7.txt',sep='\t',skiprows=(0),header=None,names=my_cols, engine='python')
#print df
df = df.astype(float)
